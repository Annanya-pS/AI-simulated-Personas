  ...
  return (
    <div className="relative group w-full my-2 max-w-full overflow-x-auto rounded-lg bg-gray-900" style={{ WebkitOverflowScrolling: 'touch' }}>
      <button
        ...
        className="absolute top-2 right-2 z-20 p-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-md transition-all duration-200 flex items-center gap-1 text-xs"
        title="Copy code"
      >
...
const MarkdownMessage: React.FC<{ content: string; isUser: boolean }> = ({ content, isUser }) => {
  ...
  return (
    <div className="prose prose-sm max-w-none prose-headings:text-gray-200 prose-p:text-gray-300 prose-strong:text-white prose-code:bg-gray-800 prose-code:text-gray-200 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:text-sm overflow-hidden prose-li:text-gray-300 prose-ul:text-gray-300 prose-ol:text-gray-300">
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 bg-slate-800/80 backdrop-blur-sm border border-slate-700 rounded-lg hover:bg-slate-700 transition-colors text-sm"
      >
        <span className="font-medium text-gray-200">{selectedOption.label}</span>
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute top-full right-0 mt-1 w-64 bg-slate-800 rounded-lg border border-slate-700 shadow-lg z-20 overflow-hidden">
            {modelOptions.map(option => (
              <button
                ...
                className={`w-full text-left px-4 py-3 hover:bg-slate-700 transition-colors ${selectedModel === option.value ? 'bg-slate-700 border-l-4 border-indigo-400' : ''
                  }`}
              >
...
const ChatApp: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-950">
      <div className="p-4 bg-slate-900/80 backdrop-blur-sm border-b border-white/10 flex items-center justify-between sticky top-0 z-50">
        <button
          onClick={toggleSidebar}
          className="p-2 rounded-lg bg-white/5 text-purple-400 hover:bg-white/10"
        ...
        <div className="flex-1 flex items-center justify-center">
          <h1 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent truncate">
            {selectedPersona ? selectedPersona.name : 'Your Persona'}
          </h1>
        </div>
      ...
      <div className="flex flex-1 min-h-0">
        <aside
          className={`hidden md:flex flex-col z-30 overflow-hidden
            ...
            bg-slate-900/90 backdrop-blur-sm border-r border-white/10 shadow-lg`}
        >
                ...
                {selectedPersona && (
                  <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-200 text-sm">{selectedPersona.name}</h3>
                        <p className="text-xs text-purple-400">{selectedPersona.role}</p>
                      </div>
                    </div>
                    <button onClick={handleBackToPersonas} className="flex items-center gap-1 text-xs text-gray-400 hover:text-purple-400 transition-colors">
                      <FiArrowLeft size={12} />
        ...
        <div className={`md:hidden fixed inset-y-0 left-0 z-40 w-72 transform ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out bg-slate-900/95 backdrop-blur-sm border-r border-white/10 shadow-lg`}>
          <div className="p-4 flex items-center justify-between border-b border-white/10">
            <h2 className="font-bold text-purple-400">Your Persona</h2>
            <button onClick={() => setMobileSidebarOpen(false)} className="p-2 rounded-lg bg-white/5 text-purple-400" aria-label="Close sidebar">
              <FiX size={20} />
            </button>
          </div>
            ...
            {selectedPersona && (
              <div className="bg-white/5 rounded-xl p-4 border border-white/10 mb-4">
                <div className="flex items-center gap-3 mb-2">
                  <img src={selectedPersona.image} alt={selectedPersona.name} className="w-10 h-10 rounded-full object-cover border-2 border-purple-200" />
                  <div>
                    <h3 className="font-bold text-gray-200 text-sm leading-tight">{selectedPersona.name}</h3>
                    <p className="text-xs text-purple-400 mt-0.5">{selectedPersona.role}</p>
                  </div>
                </div>
                <button onClick={handleBackToPersonas} className="flex items-center gap-1 text-xs text-gray-400 hover:text-purple-400 transition-colors">
                  <FiArrowLeft size={12} />
        ...
        <div className="flex-1 flex flex-col min-h-0">
                ...
                {messages.length === 0 && (
                  <div className="text-center mt-10 md:mt-20">
                    <h2 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent mb-4">
                      {selectedPersona ? `Chat with ${selectedPersona.name}` : 'Welcome to Your Persona'}
                    </h2>
                    {selectedPersona && (
                      <div className="max-w-2xl mx-auto">
                        <p className="text-gray-400 text-base md:text-lg mb-6">You're now chatting with {selectedPersona.name}, {selectedPersona.role}</p>
                        <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-4 md:p-6 mb-8">
                          <p className="text-gray-300 leading-relaxed">{selectedPersona.personality}</p>
                        </div>
                      </div>
                    )}
                    <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                      <div className="p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:shadow-md transition-shadow cursor-pointer">
                        <h3 className="font-semibold text-purple-400 mb-2">💡 Ask me anything</h3>
                        <p className="text-sm text-gray-400">I can help with coding, writing, analysis, and more</p>
                      </div>
                      <div className="p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:shadow-md transition-shadow cursor-pointer">
                        <h3 className="font-semibold text-purple-400 mb-2">🚀 Get started</h3>
                        <p className="text-sm text-gray-400">Try asking about a project or problem you're working on</p>
                      </div>
                    </div>
                  </div>
                )}
                {messages.map(msg => (
                  <div key={msg.id} className={`flex gap-2 md:gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[90vw] md:max-w-[85%] rounded-xl px-2 py-2 md:px-3 ${msg.role === "user"
                      ...
                      : "bg-slate-800 text-gray-100 rounded-tl-sm border border-slate-700"}`}
                    >
                      <div className="w-full overflow-x-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
          ...
          <div className="border-t border-white/10 bg-slate-900/50 backdrop-blur-sm p-4 md:p-6">
            <div className="max-w-4xl mx-auto flex gap-3 md:gap-4">
              <div className="flex-1 relative">
                <input
                  ...
                  disabled={isLoading}
                  className="w-full text-gray-100 border border-white/10 rounded-2xl px-4 py-3 md:px-6 md:py-4 pr-12 md:pr-14 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:opacity-50 bg-slate-800/80 backdrop-blur-sm shadow-sm placeholder-gray-500"
                />
                {wordCount > maxWords && (
                ...
                <button
                  ...
                  className={`absolute right-2 md:right-3 top-2 w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center transition-all duration-200 ${inputValue.trim() && !isLoading && wordCount <= maxWords
            ...
            <p className="text-center text-gray-500 text-xs md:text-sm mt-3 md:mt-4">
              Your Persona can make mistakes. Consider checking important information.
            </p>
          </div>
        </div>
      </div>
